...
_ARCHIVE_MAX_BYTES = 4 * 1024 * 1024
...
class StoryStore:
    ...
    def export_project(self, story_id: str) -> Path:
        """Create a project-only portable archive; media never leaves its owner root."""
        with self._lock:
            ...
            manifest = json.dumps({"schema_version": 1, "project_sha256": hashlib.sha256(encoded).hexdigest()}, sort_keys=True).encode()
    ...
    def import_project(self, source: Path) -> dict[str, object]:
        ...
        try:
            with zipfile.ZipFile(source) as archive:
                ...
                if len(entries) != 2 or {item.filename for item in entries} != {"project.json", "manifest.json"}:
                ...
                if not isinstance(manifest, dict) or manifest.get("schema_version") != 1 or manifest.get("project_sha256") != hashlib.sha256(encoded).hexdigest():
