...
def generate_video(
    ...
    # --- SynVid additions (see NOTICE.md) ---
    ...
    # Load tokenizer. SynVid always passes a local pinned path (see NOTICE.md);
    # the upstream "google/umt5-xxl" repo-id fallback only serves the
    # standalone CLI entry point below.
    from transformers import AutoTokenizer

    tokenizer = AutoTokenizer.from_pretrained(tokenizer_path or "google/umt5-xxl")
    ...
    for i, t in enumerate(range(steps)):
        if cancelled is not None and cancelled():
            raise InterruptedError("generation cancelled")
        if progress is not None:
            progress(0.1 + 0.8 * i / steps, f"denoising step {i + 1}/{steps}")
    ...
    from ..ltx_2_shared.tiling import TilingConfig
